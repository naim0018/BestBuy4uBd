  ...
  return (
    <section className="relative h-[60vh] w-full overflow-hidden flex items-center">
      ...
      <div 
        ...
        style={{ 
          backgroundImage: backgroundImage ? `url(${backgroundImage})` : 'linear-gradient(135deg, #124e66 0%, #2e3944 100%)' 
        }}
      >
      ...
      <div className="container mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
        <AnimatedContainer direction="right" className="max-w-2xl text-white space-y-6">
          <div className="space-y-2">
            {hasDiscount && (
              <div className="flex items-center gap-2 text-primary-green mb-2">
                <Sparkles className="w-5 h-5 animate-pulse" />
                <span className="text-sm font-bold uppercase tracking-widest">Limited Time Offer</span>
              </div>
            )}
          ...
          <div className="flex flex-wrap gap-4 pt-4">
            <Button
              ...
              color="primary"
              variant="shadow"
              endContent={<ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition" />}
              onPress={onShopNow}
            ...
            <div className="flex flex-col justify-center">
              <div className="flex items-center gap-2">
                <span className="text-3xl font-bold font-mono">৳{discountedPrice.toLocaleString()}</span>
                {hasDiscount && (
                  <span className="text-xl text-gray-400 line-through decoration-red-500/50">৳{regularPrice.toLocaleString()}</span>
                )}
              </div>
            </div>
          </div>
        ...
        {hasDiscount && (
          <AnimatedContainer direction="left" delay={0.2} className="hidden lg:block">
            <div className="relative">
              ...
              <div className="relative bg-white/10 backdrop-blur-xl border border-white/20 p-8 rounded-full w-48 h-48 flex flex-col items-center justify-center rotate-12 shadow-2xl">
                <span className="text-gray-200 text-sm font-bold uppercase tracking-tighter">Save</span>
                <span className="text-5xl font-black text-primary-green">{savingsPercent}%</span>
                <span className="text-gray-200 text-xs font-medium">Extra Off</span>
              </div>
            </div>
          </AnimatedContainer>
        )}
